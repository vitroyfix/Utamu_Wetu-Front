...
import { Search, User, Heart, ShoppingCart, ChevronDown, Phone, Menu, X } from 'lucide-react';
    ...
    <header className="w-full bg-white font-sans border-b border-gray-100 sticky top-0 z-[999]">
      ...
      <div className="container mx-auto px-4 py-4 flex items-center justify-between gap-2 md:gap-6 h-20 relative z-[1001] bg-white">
        <div className="flex items-center gap-1 shrink-0">
          ...
          {/* Logo Section with groceries.webp */}
        ...
        <div className="flex items-center gap-3 md:gap-8 shrink-0">
          {/* Account */}
          <div className="hidden xs:flex items-center gap-1 md:gap-2 cursor-pointer group transition-all duration-300 hover:scale-110">
            <User size={18} className="text-gray-700 group-hover:text-[#3BB77E] md:w-6 md:h-6" />
            <span className="hidden xl:block text-[15px] font-medium text-gray-700 group-hover:text-[#3BB77E]">Account</span>
          </div>

          {/* Wishlist */}
          <div className="flex items-center gap-1 md:gap-2 cursor-pointer group transition-all duration-300 hover:scale-110">
            ...
            <span className="hidden xl:block text-[15px] font-medium text-gray-700 group-hover:text-[#3BB77E]">Wishlist</span>
          </div>
      ...
      >
        <div className="flex flex-col p-6 space-y-4">
          {navLinks.map((link) => (
            ...
            >
              {link.name}
              {link.hasDropdown && <ChevronDown size={18} className="text-gray-300" />}
            </Link>
          ))}
        </div>
      </div>
